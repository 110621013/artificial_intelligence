...
def example():
    ...
    model = LinearRegression(feature_size * input_date_data_size, 1)
    learning_rate = 0.01
    ...
    output_df.to_csv(os.path.join('.','hw1_StockPricePrediction','hw1_submission.csv'), index=False)
...
def multi_linear():
    ...
    class LinearRegression_v2(nn.Module):
        def __init__(self, input_dim, output_dim): #32,1
            ...
            # 定義每層用什麼樣的形式
            self.layer1 = torch.nn.Linear(input_dim, 1200)
...
def test_look_jena():
    #create x, y dataset, x is now temp and y is next time temp(y for compare:))
    def create_newdataset(dataset, forward):
        x, y = [], []
    ...
    #parameter
    datadays = 31 #data scale(10 mins a data)
    split = 0.5 #dataset=>train, test
    forward = 6*24 #24hr predict


    move = 0
    #get 
    datasource_path = 'C:\\Users\\user\\Desktop\\done\\暑期實習\\暑期實習所有code\\summer_intern_deeplearning_VMplusWindows\\jena_climate\\'
    datasource_name = 'jena_climate_2009_2016.csv' #per 10mins 1 data
    dataframe = pd.read_csv(datasource_path+datasource_name, usecols=[2], engine='python')
    dataset = dataframe.values.astype('float32')[move:move+6*24*datadays] #turn to numpy.ndarray (datadays)
    print('dataset', type(dataset), dataset.shape) #<class 'numpy.ndarray'> (4464, 1)
    #normalize
    scaler = MinMaxScaler(feature_range=(0, 1))
    dataset = scaler.fit_transform(dataset)
    #split to train and test(split vs 1-split)
    dataset_split_point = int(len(dataset)*split)
    train, test = dataset[0:dataset_split_point, :], dataset[dataset_split_point:len(dataset), :]
    print('I train :', len(train), 'test:', len(test)) #2232

    train_now, train_next = create_newdataset(train, forward)
    test_now, test_next = create_newdataset(test, forward)
    print('============>', train_now.shape, train_next.shape) #(2087, 144) (2087,)
    ...
    print('============>', train_now.shape, train_next.shape) #(2087, 1, 144) (2087,)
